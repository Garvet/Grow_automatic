...
#if defined (NEW_VERSION_NOT_WORKING)
//   ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
// ----- ----- ----- ----- -----  Packet_Sensor  ----- ----- ----- ----- -----
//   ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
uint8_t Packet_Sensor::set_packet_data(LoRa_packet& packet, uint8_t *amt, uint8_t *param, uint8_t *id, uint32_t *data) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (field_bit[2].get_value(&setting_) != 0) {
            error = field_byte.set_value(id[i], packet, last_filled_byte);
            // error = field_byte.set_value(id[i], &(packet[last_filled_byte]), (SIZE_LORA_PACKET_MAX_LEN - last_filled_byte));
            if (error)
                return 3;
            ++last_filled_byte;
        }
...
/// --- Чтение из пакета ---
// получить из пакета параметры и данные
uint8_t Packet_Sensor::get_packet_data(LoRa_packet& packet, uint8_t *amt, uint8_t *param, uint8_t *id, uint32_t *data) {
    uint8_t count = 1;
    last_read_byte = PACKET_HEADER_SIZE;
    // количество параметров
...
uint8_t Packet_Sensor::get_size_by_packet(LoRa_packet& packet, uint8_t *amt, uint8_t *param, uint8_t *size_data) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // данные
        last_read_byte = sensor_param_data[param[i]];
        size_data[i] = sensor_param_data[param[i]];
    }
    return 0;
}
#endif
