...
bool Grow_device::set_dev_state(uint8_t num, bool state) {
    if(get_count_component() <= num)
        return true;
...
bool Grow_device::get_dev_state(uint8_t num, bool &result) {
    if(get_count_component() <= num)
        return true;
    result = component_[num].get_state();
    return false;
}
bool Grow_device::set_dev_state(std::vector<bool> state) {
...
std::vector<bool> Grow_device::get_dev_state() {
