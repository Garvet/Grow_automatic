...
// #define AUTO_REG
    ...
    if(1) {
...
#if defined( BUILD_S1_D0_15s_A3 )
            {0x03, 0x07, 0x12, 0x22, 0x0c, 0x0d, 0xc2, 0xd2, 0x4c, 0x01, 0xc4, 0x02},
#endif

#if defined( BUILD_S2_D0_60s_BASEMENT ) || defined( BUILD_S2_D1_60s_BASEMENT ) || defined( BUILD_S4_D1_60s_BASEMENT )
            // BASEMENT
...
#if defined( BUILD_S0_D1_15s_A3 )
            {0x03, 0x07, 0x12, 0x22, 0x0c, 0x0d, 0xc2, 0xd2, 0x4c, 0x01, 0xc4, 0x02},
#endif

#if defined( BUILD_S0_D1_60s_BASEMENT ) || defined( BUILD_S2_D1_60s_BASEMENT ) || defined( BUILD_S4_D1_60s_BASEMENT )
            // BASEMENT
            {0x60, 0x86, 0xcf, 0x93, 0x34, 0x2b, 0x73, 0x2b, 0x2d, 0x4a, 0x42, 0x76}, // [Air_humidity, Air_temperature, Indicator_eCO2]
#endif
        ...
        for(int i = 0; (i < __GCM__.sensors_.size()) && (i < array_size(sensors_id)); ++i) {
            __GCM__.sensors_[i].set_active(2);
            __GCM__.sensors_[i].set_system_id(sensors_id[i]);
        }
        for(int i = 0; (i < __GCM__.devices_.size()) && (i < array_size(devices_id)); ++i) {
